...
type OpportunityMetrics = NonNullable<
    ValidatedQueryOverviewSource['opportunity_metrics']
>;
...
function addLegacyOpportunityMetrics(
    ...
    queries.forEach((query) => {
        ...
        const currentMaximum =
            maximumSearchVolumeByTerritory.get(query.territory) ?? 0;

        maximumSearchVolumeByTerritory.set(
            query.territory,
            Math.max(currentMaximum, searchVolume),
        );
    });
    ...
    return queries.map((query) => {
        ...
        const keywordDifficultyUsed = clampPercentage(
            keywordDifficultyOriginal ?? fallbackKeywordDifficulty,
        );
        const demandScore =
            maximumTerritorySearchVolume > 0
                ? Math.log1p(searchVolumeUsed) /
                  Math.log1p(maximumTerritorySearchVolume)
                : 0;
        ...
        return {
            ...
            opportunity_metrics: {
                ...
                opportunity_score:
                    0.7 * demandScore * 100 + 0.3 * attainabilityScore * 100,
                ...
                keyword_difficulty_was_imputed:
                    keywordDifficultyOriginal === null,
...
function formatContentPlanDemandType(
    territory: 'problem_demand' | 'solution_demand',
) {
    return territory === 'problem_demand'
        ? 'Problem Demand'
        : 'Solution Demand';
}
...
function getRankingDetails(
    ...
    const pageOneQueries = uniqueQueryRankings.filter(
        (ranking) => ranking.position >= 1 && ranking.position <= 10,
    ).length;
    const pageTwoQueries = uniqueQueryRankings.filter(
        (ranking) => ranking.position >= 11 && ranking.position <= 20,
    ).length;
    const lowerRankingQueries = uniqueQueryRankings.filter(
        (ranking) => ranking.position >= 21,
    ).length;
    ...
    return {
        rankingFootprint: {
            ...
            pageOneShare:
                matchedQueries > 0
                    ? (pageOneQueries / matchedQueries) * 100
                    : 0,
        },
...
export async function getSerpReportData(
    slug: string,
): Promise<SerpReportData | null> {
    noStore();
    ...
    if (error) {
        throw new Error(
            `Unable to load SERP report data for "${normalizedSlug}": ${error.message}`,
        );
    }
    ...
    const queryAnalysisSummary = queryAnalysisSummaryDataSchema.parse({
        ...
        medianKeywordDifficulty: data.median_keyword_difficulty,
        averageMonthlySearchVolume:
            data.validated_average_monthly_search_volume,
        medianMonthlySearchVolume: data.validated_median_monthly_search_volume,
    });

    const competitorScope = competitorLandscapeScopeSourceSchema.parse(
        data.competitor_scope,
    );
    const competitorSummary = competitorLandscapeSummarySourceSchema.parse(
    ...
    const broadestCoverage =
        sourceCompetitors.length > 0
            ? sourceCompetitors.toSorted((a, b) => {
                  if (
                      a.query_coverage_percentage !==
                      b.query_coverage_percentage
                  ) {
                      return (
                          b.query_coverage_percentage -
                          a.query_coverage_percentage
    ...
    const competitorLandscape = competitorLandscapeDataSchema.parse({
        ...
        pageOneCompetitors: sourceCompetitors.filter(
            (competitor) =>
                Number.isFinite(competitor.median_position) &&
                competitor.median_position <= 10,
        ).length,
        visibilityLeader: visibilityLeader
            ? {
                  domain: visibilityLeader.domain,
                  queryCoveragePercentage:
                      visibilityLeader.query_coverage_percentage,
        ...
        broadestCoverage: broadestCoverage
            ? {
                  ...
                  queryCoveragePercentage:
                      broadestCoverage.query_coverage_percentage,
              }
            : null,
        competitors: sourceCompetitors.map((competitor) => {
            const rankingDetails = getRankingDetails(
                competitor.query_positions,
            );
            ...
            return {
                ...
                estimatedTraffic:
                    competitor.estimated_traffic_from_analyzed_queries,
    ...
    const sourceQueries = addLegacyOpportunityMetrics(
        validatedQueryOverviewSourceArraySchema.parse(
            data.validated_query_rows,
        ),
        analysisScope.medianKeywordDifficulty,
    );
    const contentPlanItems = contentPlanItemSourceArraySchema
        .parse(data.content_plan_items ?? [])
        .toSorted((a, b) => {
    ...
    const searchMarket = reportOverviewSearchMarketSourceSchema.parse(
        data.search_market,
    );
    const contentPlanSummary =
        reportOverviewContentPlanSummarySourceSchema.parse(
    ...
    const contentPlan = contentPlanDataSchema.parse({
        ...
        recommendations: contentPlanItems.map((item) => {
            ...
            const demoDraftPreview = getDemoDraftPreview(
                normalizedSlug,
            ...
            return {
                ...
                draftPreview:
                    item.draft_preview ?? demoDraftPreview?.draftPreview,
                draftCategory:
                    item.draft_category ?? demoDraftPreview?.draftCategory,
                draftReadTimeMinutes:
                    item.draft_read_time_minutes ??
                    demoDraftPreview?.draftReadTimeMinutes,
                draftPreviewHeading:
                    item.draft_preview_heading ??
                    demoDraftPreview?.draftPreviewHeading,
                draftPreviewContinuation:
                    item.draft_preview_continuation ??
                    demoDraftPreview?.draftPreviewContinuation,
                ...
                searchIntent: formatSearchIntent(
                    item.query_metrics.search_intent.main,
                ),
                ...
                topTenBenchmark: item.query_metrics.average_top_10
                    ? {
                          averageBacklinks:
                              item.query_metrics.average_top_10.backlinks,
                          averageReferringDomains:
                              item.query_metrics.average_top_10
                                  .referring_domains,
                          averageDomainRank:
                              item.query_metrics.average_top_10
                                  .main_domain_rank,
    ...
    const scoredOpportunities = scoredOpportunitySourceArraySchema.parse(
        data.scored_opportunities ?? [],
    );
    const problemDemandPercentage =
        queryAnalysisSummary.total > 0
            ? Math.round(
                  (queryAnalysisSummary.problemDemand /
                      queryAnalysisSummary.total) *
                      100,
              )
            : 0;
    const solutionDemandPercentage =
        queryAnalysisSummary.total > 0 ? 100 - problemDemandPercentage : 0;
    const reportOverview = reportOverviewDataSchema.parse({
        ...
        broadestCoverage: competitorLandscape.broadestCoverage
            ? {
                  ...
                  percentage:
                      competitorLandscape.broadestCoverage
                          .queryCoveragePercentage,
              }
            : null,
    ...
    const selectedByQueryId = new Map(
        contentPlanItems.map((item) => [
            ...
            {
                opportunityScore:
                    item.opportunity_metrics?.opportunity_score ?? null,
    ...
    const queryOverview = queryOverviewDataSchema
        .parse(
            sourceQueries.map((item) => ({
                id: item.query_id,
                query: item.query,
                demandType:
                    item.territory === 'problem_demand'
                        ? 'Problem'
                        : 'Solution',
                searchIntent: formatSearchIntent(
                    item.metrics.search_intent.main,
                ),
                ...
                quarterlyTrend:
                    item.metrics.search_volume_trend?.quarterly ?? null,
                ...
                averageBacklinks:
                    item.metrics.average_top_10?.backlinks ?? null,
                averageReferringDomains:
                    item.metrics.average_top_10?.referring_domains ?? null,
        ...
        sourceQueries.map((item) => {
            ...
            const status = selected
                ...
                : scored
                  ? 'scored'
                  : 'validated';
            const graphMetrics = getOpportunityGraphMetrics(
                item.opportunity_metrics,
            );

            if (!graphMetrics.scoreMatchesMethodology) {
                throw new Error(
                    `Opportunity score for query "${item.query_id}" does not match the Tavyn v2.0 methodology.`,
                );
            }
            ...
            return {
                ...
                keywordDifficultyUsed:
                    item.opportunity_metrics.keyword_difficulty_used,
                keywordDifficultyWasImputed:
                    item.opportunity_metrics.keyword_difficulty_was_imputed,

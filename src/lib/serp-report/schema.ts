    ...
    .superRefine((data, ctx) => {
        if (data.queriesDiscovered < data.queriesValidated) {
            ctx.addIssue({
                ...
                message:
                    'queriesDiscovered must be greater than or equal to queriesValidated.',
        ...
        if (data.queriesValidated < data.contentOpportunitiesScored) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message:
                    'queriesValidated must be greater than or equal to contentOpportunitiesScored.',
                path: ['contentOpportunitiesScored'],
            });
        }

        if (
            data.contentOpportunitiesScored <
            data.contentRecommendationsSelected
        ) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message:
                    'contentOpportunitiesScored must be greater than or equal to contentRecommendationsSelected.',
...
export type QueryAnalysisSummaryData = z.infer<
    typeof queryAnalysisSummaryDataSchema
>;
...
export const competitorLandscapeCompetitorSourceArraySchema = z.array(
    competitorLandscapeCompetitorSourceSchema,
);
...
export type CompetitorLandscapeData = z.infer<
    typeof competitorLandscapeDataSchema
>;
...
export const validatedQueryOverviewSourceArraySchema = z.array(
    validatedQueryOverviewSourceSchema,
);

export type ValidatedQueryOverviewSource = z.infer<
    typeof validatedQueryOverviewSourceSchema
>;
...
export const contentPlanItemSourceArraySchema = z
    .array(contentPlanItemSourceSchema)
    .length(3);
...
export type ContentPlanRecommendation = z.infer<
    typeof contentPlanRecommendationSchema
>;
...
export const scoredOpportunitySourceArraySchema = z.array(
    scoredOpportunitySourceSchema,
);
...
export const searchOpportunityPointArraySchema = z.array(
    searchOpportunityPointSchema,
);

export type SearchOpportunityPoint = z.infer<
    typeof searchOpportunityPointSchema
>;

...
export function getOpportunityGraphMetrics(metrics: OpportunityMetricsSource) {
  ...
  const rankingAttainability = clampPercentage(metrics.attainability_score * 100);
  const opportunityScore = clampPercentage(metrics.opportunity_score);
  const expectedScore = 0.7 * relativeSearchDemand + 0.3 * rankingAttainability;
  ...
  return {
    ...
    territoryP95SearchVolume:
      metrics.territory_p95_search_volume ?? metrics.maximum_territory_search_volume ?? 0,
    scoreMatchesMethodology: Math.abs(opportunityScore - expectedScore) <= SCORE_ROUNDING_TOLERANCE,
  };
}

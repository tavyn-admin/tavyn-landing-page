...
function RankingFootprint({ competitor }: { competitor: CompetitorRow }) {
    const {
        ...
        pageTwoQueries,
        lowerRankingQueries,
    } = competitor.rankingFootprint;
    const queryLabel = matchedQueries === 1 ? 'query' : 'queries';

    if (matchedQueries === 0) {
        return (
            <p className={styles.unavailableDetail}>
                Detailed query rankings were not available for this competitor.
            </p>
        );
    }
    ...
    return (
        <section className={styles.detailsSection}>
            ...
            <p className={styles.footprintSummary}>
                {competitor.domain} ranks in positions 1–10 for{' '}
                {formatInteger(pageOneQueries)} of the{' '}
                {formatInteger(matchedQueries)} analyzed {queryLabel} where it
                appears.
            </p>
        ...
        <section className={styles.detailsSection}>
            ...
            <div className={styles.queryRankingList}>
                {competitor.strongestQueryRankings.map((ranking) => (
                    <div
                        className={styles.queryRankingRow}
                        key={`${competitor.domain}-${ranking.query}-${ranking.position}`}
                    >
...
function CompetitorDetails({ competitor }: { competitor: CompetitorRow }) {
    const hasDetails =
        competitor.rankingFootprint.matchedQueries > 0 ||
        competitor.strongestQueryRankings.length > 0;
    ...
    return (
        <div className={styles.competitorDetails}>
            ...
            {hasDetails ? (
                ...
                <p className={styles.unavailableDetail}>
                    Detailed query rankings were not available for this
                    competitor.
                </p>
            )}
        </div>
...
export default function CompetitorList({
    ...
    return (
        <div
            ...
            role="table"
            aria-label="Competitor visibility across validated queries"
        >
            ...
            >
                ...
                {competitors.length > 0 ? (
                    competitors.map((competitor) => {
                        ...
                        return (
                            <article
                                ...
                                data-leader={
                                    leaderDomains.includes(competitor.domain)
                                        ? 'true'
                                        : undefined
                                }
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span className={styles.metricCell}>
                                        {formatInteger(
                                            competitor.keywordsRankedCount,
                                        )}
                                    </span>
                                    <span className={styles.metricCell}>
                                        {formatCoverage(
                                            competitor.queryCoveragePercentage,
                                        )}
                                    </span>
                                    <span className={styles.metricCell}>
                                        {formatPosition(
                    ...
                    <div className={styles.emptyMessage}>
                        No competitor domains were available for this report.
                    </div>
                )}
            </div>
        </div>

        ...
        >
            ...
            <dl className={styles.metricStrip}>
                <Metric
                    ...
                    value={formatInteger(recommendation.monthlySearchVolume)}
                />
                <Metric
                    label="Keyword Difficulty"
                    value={formatNumber(recommendation.keywordDifficulty)}
                />
                <Metric
                    label="Opportunity Score"
                    value={formatNumber(recommendation.opportunityScore)}
                />
                <Metric
                    label="Search intent"
                    value={recommendation.searchIntent}
                />
            </dl>
            ...
            >
                ...
                <div className={styles.evidenceContent}>
                    <dl className={styles.evidenceGrid}>
                        <Metric
                            label="Demand territory"
                            value={recommendation.demandType}
                        />
                        <Metric
                            label="Average candidate score"
                            value={formatNumber(averageOpportunityScore)}
                        />
                        <Metric
                            label="Paid competition"
                            value={formatPaidCompetition(
                                recommendation.paidCompetition,
                            )}
                        />
                        <Metric
                            label="1-month momentum"
                            value={formatSignedPercentage(
                                recommendation.searchMomentum.monthly,
                            )}
                        />
                        <Metric
                            label="3-month momentum"
                            value={formatSignedPercentage(
                                recommendation.searchMomentum.quarterly,
                            )}
                        />
                        <Metric
                            label="12-month momentum"
                            value={formatSignedPercentage(
                                recommendation.searchMomentum.yearly,
                            )}
                        />
                    </dl>
                            ...
                            <dl className={styles.benchmarkGrid}>
                                <Metric
                                    ...
                                    value={formatNullableInteger(
                                        benchmark.averageBacklinks,
                                    )}
                                />
                                <Metric
                                    label="Average referring domains"
                                    value={formatNullableInteger(
                                        benchmark.averageReferringDomains,
                                    )}
                                />
                                <Metric
                                    label="Average domain rank"
                                    value={formatNullableInteger(
                                        benchmark.averageDomainRank,
                                    )}
                                />
                            </dl>
                        </section>
                    ...
                    <section className={styles.rankingSection}>
                        <h3>
                            ...
                            <span>{recommendation.rankingPages.length}</span>
                        </h3>
                        {recommendation.rankingPages.length === 0 ? (
                            <p className={styles.rankingEmptyState}>
                                No ranking pages were returned for this query.
                            </p>
                        ) : (
                            <div
                                className={styles.rankingTableWrap}
                                tabIndex={0}
                            >
                                <table className={styles.rankingTable}>
                                    ...
                                    <tbody>
                                        {recommendation.rankingPages.map(
                                            (page, pageIndex) => (
                                                <tr
                                                    key={`${page.position}-${page.domain}-${page.url}-${pageIndex}`}
                                                >
                                                    <td>
                                                        {formatInteger(
                                                            page.position,

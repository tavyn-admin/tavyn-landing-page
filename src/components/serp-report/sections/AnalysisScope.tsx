...
function MetricItem({ label, value, description, index, valueRef }: MetricItemProps) {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        const tick = (timestamp: number) => {
          ...
          counters.forEach(({ element, target, suffix, delay, duration }) => {
            if (!element) return;

            const progress = Math.min(1, Math.max(0, (timestamp - startedAt - delay) / duration));
            const easedProgress = 1 - Math.pow(1 - progress, 3);
            const displayedValue = progress === 1 ? target : Math.round(target * easedProgress);
  ...
  return (
    <div ref={rootRef} className={`${pageStyles.page} ${styles.root}`}>
      <header className={pageStyles.header}>
        <h1 className={`${pageStyles.title} ${styles.sectionTitle}`}>{possessiveCompanyName} Analysis Scope</h1>
        <p className={`${pageStyles.subtitle} ${styles.sectionDescription}`}>
        ...
        >
          ...
          <div className={`${styles.callout} ${styles.discoveredCallout}`}>
            ...
            <div className={styles.calloutValue}>
              <span ref={(element) => { milestoneValueRefs.current[0] = element; }} aria-hidden="true">
                {formatNumber(analysisCoverage.queriesDiscovered)}
              </span>
              <span className={styles.srOnly}>{formatNumber(analysisCoverage.queriesDiscovered)}</span>
            </div>
          </div>
          <div className={`${styles.callout} ${styles.scoredCallout}`}>
            <div className={styles.calloutLabel}>Content opportunities scored</div>
            <div className={styles.calloutValue}>
              <span ref={(element) => { milestoneValueRefs.current[1] = element; }} aria-hidden="true">
                {formatNumber(analysisCoverage.contentOpportunitiesScored)}
              </span>
              <span className={styles.srOnly}>{formatNumber(analysisCoverage.contentOpportunitiesScored)}</span>
            </div>
          </div>
          <div className={`${styles.callout} ${styles.selectedCallout}`}>
            <div className={styles.calloutLabel}>Priority opportunities selected</div>
            <div className={styles.calloutValue}>
              <span ref={(element) => { milestoneValueRefs.current[2] = element; }} aria-hidden="true">
          ...
          <div className={`${styles.connector} ${styles.selectedConnector}`}>
            ...
            <span className={styles.selectedLeadIn} aria-hidden="true" />
            <span className={styles.finalPointPulse} aria-hidden="true" />
          </div>
        </div>

        <section className={`${pageStyles.keySummary} ${styles.conclusion}`}>
          <h2 className={styles.conclusionTitle}>Tavyn examines {possessiveCompanyName} full search landscape.</h2>

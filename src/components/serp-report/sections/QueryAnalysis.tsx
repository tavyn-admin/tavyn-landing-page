...
function getDemandConcentrationPhrase(summary: QueryAnalysisSummaryData) {
    if (
        summary.averageMonthlySearchVolume === 0 &&
        summary.medianMonthlySearchVolume === 0
    ) {
    ...
    const volumeRatio =
        summary.averageMonthlySearchVolume / summary.medianMonthlySearchVolume;
...
export default function QueryAnalysis({
    ...
    const difficultyClassification = getDifficultyClassification(
        summary.medianKeywordDifficulty,
    );
        ...
        <div className={`${pageStyles.page} ${styles.root}`}>
            <div className={`${pageStyles.primaryContent} ${styles.content}`}>
                <section
                    ...
                    aria-label="Search Opportunity Map"
                >
                ...
                <div ref={queryListRef} className={styles.fadeBlock}>
                    <QueryList
                        queries={queries}
                        opportunityPoints={opportunityPoints}
                    />
                </div>

                <section
                    ref={summaryRef}
                    className={`${pageStyles.keySummary} ${styles.fadeBlock}`}
                >
                    <h2>
                        Not every query is worth chasing for {companyName}.
                        Tavyn found the ones that are.
                    </h2>
                    <p>
                        Across {formatNumber(summary.total)} validated queries,
                        search demand {concentrationPhrase}. {intentSentence} A
                        median difficulty score of{' '}
                        {formatNumber(summary.medianKeywordDifficulty)}{' '}
                        indicates {difficultyClassification}. Tavyn identified{' '}
                        {formatNumber(priorityOpportunityCount)} priority
                        opportunities with the strongest combination of search
                        demand, ranking difficulty, search intent, and relevance
                        to your business. These are the pages you should publish
                        first.
                    </p>
                </section>
            </div>

...
export function clusterOpportunityPoints(
  ...
  const pointsAreClose = (first: SearchOpportunityPoint, second: SearchOpportunityPoint) => {
    const deltaX = Math.abs(first.rankingAttainability - second.rankingAttainability);
    const deltaY = Math.abs(first.relativeSearchDemand - second.relativeSearchDemand);
  ...
  sortedPoints.forEach((point) => {
    ...
    const matchingGroup = groups.find((group) => group.every((member) => pointsAreClose(point, member)));
  ...
  return groups.map((group): OpportunityCluster => {
    ...
    const representative = group.reduce((closest, point) => {
      const closestDistance = Math.hypot(
        ((closest.rankingAttainability - centroid.x) / 100) * renderedPlotWidth,
        ((closest.relativeSearchDemand - centroid.y) / 100) * renderedPlotHeight
      );
      const pointDistance = Math.hypot(
        ((point.rankingAttainability - centroid.x) / 100) * renderedPlotWidth,
        ((point.relativeSearchDemand - centroid.y) / 100) * renderedPlotHeight
      );

      return pointDistance < closestDistance ||
        (pointDistance === closestDistance && point.queryId.localeCompare(closest.queryId) < 0)
        ? point
        : closest;
    }, group[0]);
    ...
    return {
      ...
      points: [...group].sort((a, b) => {
        ...
        if (a.recommendationRank !== b.recommendationRank) {
          return (a.recommendationRank ?? Number.POSITIVE_INFINITY) - (b.recommendationRank ?? Number.POSITIVE_INFINITY);
        }

        return b.opportunityScore - a.opportunityScore || a.query.localeCompare(b.query);
      }),
      ...
      opportunityScore: Math.max(...group.map((point) => point.opportunityScore)),
    };
  });
}

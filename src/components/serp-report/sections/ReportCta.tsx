      ...
      <div className={styles.content}>
        <section className={styles.conversionCopy}>
          <h2 className={styles.heading}>
            <span className={styles.introLine}>Your buyers are already searching.</span>
            <span className={styles.emphasizedLine}>It’s time they found {customerName}.</span>
          </h2>

          <form className={styles.waitlistForm} onSubmit={handleSubmit} noValidate>
              <label className={styles.emailLabel} htmlFor="report-waitlist-email">
                Work email
              </label>
              <input
                ...
                required
              />
              <button className={styles.waitlistButton} type="submit" disabled={submitting} aria-busy={submitting}>
                {submitting ? "Joining…" : "Join the waitlist + get my draft"}
              </button>
              {submitError ? (
                <p className={styles.formError} id="report-waitlist-error" role="alert">
                  {submitError}
                </p>
              ) : null}
              <p className={styles.microcopy}>
                By joining, you agree to receive the draft and Tavyn updates. Unsubscribe anytime.
              </p>
          </form>
        </section>

        <section className={styles.documentStage} aria-label={`${recommendations.length} mapped article drafts`}>
          <div className={styles.documentStack}>
            {rearRecommendations.toReversed().map((recommendation, reversedIndex) => {
              const actualIndex = rearRecommendations.length - reversedIndex;
              return (
                <article
                ...
                >
                  <span>Draft {formatDraftNumber(actualIndex)}</span>
                  <strong>{recommendation.recommendedTitle}</strong>
                </article>
              );
            })}
            ...
            {firstRecommendation ? (
              <article className={styles.frontDocument}>
                <span className={styles.documentTab}>Draft 01</span>
                <h3>{firstRecommendation.recommendedTitle}</h3>
                {firstRecommendation.draftCategory || firstRecommendation.draftReadTimeMinutes ? (
                  <p className={styles.articleMetadata}>
                    {[firstRecommendation.draftCategory, firstRecommendation.draftReadTimeMinutes
                      ? `${firstRecommendation.draftReadTimeMinutes} min read`
                      : undefined, "Article draft"]
                      .filter(Boolean)
                      .join(" · ")}
                  </p>
                ) : null}
                {draftPreview ? (
                  <div className={styles.articleBody}>
                    ...
                    {firstRecommendation.draftPreviewHeading ? (
                      <h4>{firstRecommendation.draftPreviewHeading}</h4>
                    ) : null}
                    {firstRecommendation.draftPreviewContinuation ? (
                      <div className={styles.previewText}>
                        <p>{firstRecommendation.draftPreviewContinuation}</p>
                      </div>
                    ) : null}
                  </div>
                ) : null}
                <span className={styles.reservedLabel}>Full draft available when you join the waitlist</span>
              </article>
            ) : null}
          </div>
          {remainingCount > 0 ? (
            <p className={styles.mappedCount}>
              {remainingCount} more {remainingCount === 1 ? "article" : "articles"} mapped
            </p>
          ) : null}
        </section>
      </div>
